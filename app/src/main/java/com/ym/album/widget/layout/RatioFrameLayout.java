...
public class RatioFrameLayout extends FrameLayout {
    ...
    public RatioFrameLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.RatioFrameLayout);
        String sizeRatio = typedArray.getString(R.styleable.RatioFrameLayout_sizeRatio);
        if (EmptyUtil.strNotEmpty(sizeRatio)){
            ...
            switch (arrays.length){
                ...
                case 2:
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("RatioFrameLayout array length "+arrays.length);
            }
        }
        typedArray.recycle();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (mWidthRatio != 0 && heightMeasureSpec != 0){
            ...
            // 所以我们要进行双重判断，首先判断 LayoutParams，再判断测量模式，这样就能避免因为修改了测量模式触发对宽高的重新计算，最终导致计算结果和上次计算的不
